...
async function findCampaignByName(name) {
  ...
  while (true) {
    ...
    const match = (data.results || []).find(c => (c.properties?.hs_name || '') === name);
...
async function createCampaign(name) {
  const body = {
    properties: {
      hs_name: name,
      ...(HSPROP_LAST_STATUS ? { [HSPROP_LAST_STATUS]: 'CREATED' } : {}),
    },
  };
...
async function addToTotals(campaignId, { addClicks, addImpressions, addConversions }, dateISO) {
  const current = await getTotals(campaignId);
  const next = {
    [HSPROP_TOTAL_CLICKS]:      (current.clicks || 0) + (Number(addClicks) || 0),
    [HSPROP_TOTAL_IMPRESSIONS]: (current.imps   || 0) + (Number(addImpressions) || 0),
    [HSPROP_TOTAL_CONVERSIONS]: (current.convs  || 0) + (Number(addConversions) || 0),
  };
