...
// --- Map envs to the names you actually use in your portal ---
// Prefer your provided names; fall back to older keys if present; else sensible defaults.
const HSPROP_CLICK_TOTAL =
  process.env.HSPROP_TOTAL_CLICKS ||
  process.env.HSPROP_CLICK_TOTAL ||
  'total_clicks';

const HSPROP_IMP_TOTAL =
  process.env.HSPROP_TOTAL_IMPRESSIONS ||
  process.env.HSPROP_IMP_TOTAL ||
  'total_impressions';

const HSPROP_CONV_TOTAL =
  process.env.HSPROP_TOTAL_CONVERSIONS ||
  process.env.HSPROP_CONV_TOTAL ||
  'total_conversions';

// "last processed/status" (we update these; names supplied by you)
const HSPROP_LAST_STATUS =
  process.env.HSPROP_LAST_STATUS || 'bing_last_status';

const HSPROP_LAST_PROCESSED =
  process.env.HSPROP_LAST_BING_DATE ||
  process.env.HSPROP_LAST_PROCESSED ||
  'bing_last_processed';

// We never PATCH total spend here (it's a read-only HS rollup)
...
async function createSpendItem(campaignId, { isoDate, amountMinorUnits, source }) {
  ...
  // amount expects a number (in minor currency units)
  const body = {
    name,               // string
...
async function patchCampaignProperties(campaignId, props) {
  ...
  // For totals we’ll send numbers as strings (HubSpot accepts both), and epoch as number.
  const normalized = {};
